...
## 追記 (2026-09-15): preview 付きの選択肢は「数字で動かして Enter で決める」

Claude Code 2.1.272 で、選択肢のどれかが `preview` を持つ質問 (単一選択・複数質問とも。multiSelect は対象外) は別のレイアウトで描かれる。実機の席 (80 桁) を `pane.read` で読んで計測した。

```
☐ 帯の確認
テスト質問 1: 画面の下に黒い帯のボタンは出ていましたか？
❯ 1. 出ていた                     ┌──────────────────────────────────────────┐
  2. 出ていなかったので直して     │ ┌──────────────┐                         │
     ほしい                        ├─── ✂ ─── 5 lines hidden ─────────────────┤
  3. 見ていない・もう一度見る     └──────────────────────────────────────────┘
...
| 違い | 計測 |
|---|---|
| 並び | 選択肢は幅 30 セルの列に入り、その右 (間隔 4 セル) にカーソル行の preview が枠つきで出る。長いラベルは列の中で折り返す (CJK は文字境界)。description は描かれない |
| 数字キー | **カーソルを動かすだけ**で確定しない (`onKeyDown` の `1`〜`9` は cursor move)。既にその行にあれば何も変わらない |
| 確定 | **Enter** (`return` → select)。単一質問はこれで閉じ、複数質問は次のタブへ進む。確認画面 (review) は従来どおり数字 `1` |
| 脚注 | `Enter to select · ↑/↓ to navigate · n to add notes · Esc to cancel`。複数質問では `· Tab to switch questions` が足され、80 桁では 2 行に折り返す (`Esc to cancel` だけが次の行) |
| Chat about this | 番号なしで区切り線の下に出る。`Type something` 行は出ない |
| `pane.read` | 各行を trim して空行を落とす。列位置は使えないので、preview の枠は行頭の `┌│├└` / `Notes:` で、ラベルは 4 つ以上の空白の手前で切る |

従来 (preview なし) の「数字 1 バイトで即決」は変わらない。実測は同じ席で `2` → カーソルが 2 行目へ → `Enter` → 質問が閉じ、transcript の `toolUseResult.answers` に 2 番目のラベルが記録された。複数質問は `2`・`Enter` → 次タブ → `Enter` → 確認画面 → `1` で `{"…2A": "緑", "…2B": "大きい"}`。

決めたこと: `mycmux-bridge` の `scan_ask_question` は脚注の `n to add notes` で preview レイアウトを見分けて `layout: "preview"` を返し、`answer-ask` はそのときだけ「必要なら数字でカーソルを動かし、画面で確かめてから Enter」を送る。ポケットはこれまでどおり bridge の結果だけを使う。
