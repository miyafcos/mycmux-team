...
DATA = """
MYC-001|起動・ウィンドウ|起動時の作業ディレクトリ引き継ぎ|P0|Implemented per code|Likely|src/App.tsx@getLaunchCwd;src/App.tsx@listStore.workspaces.length === 0;src/lib/ipc.ts@get_launch_cwd|ユーザーとして任意の作業ディレクトリから起動したとき、そのディレクトリを初期ワークスペースとして使いたい。|永続ワークスペースが空でlaunch cwdが取れた場合、1x1ワークスペースを作成し、タブとペインのcwdへlaunch cwdを設定する。|本体再起動が必要な実機検証は後回し。
MYC-002|起動・ウィンドウ|既存ワークスペースの起動復元|P0|Implemented per code|Likely|src/App.tsx@prepareStartupSessionGate;src/App.tsx@STARTUP_RESTORE_COMPLETE_EVENT;src/components/layout/SocketListener.tsx@useWorkspacePersist|ユーザーとして前回使っていたワークスペースとタブ構成を次回起動時に戻したい。|永続データ読込後、保存済みワークスペースを起動ゲート経由でマウントし、復元完了までスタートアップマスクを維持する。|再起動禁止のため静的棚卸しのみ。
...
MYC-008|タブ配置|タブ分割|P0|Implemented per UI/store|No|src/components/workspace/PaneHeader.tsx@Split pane;src/components/workspace/PaneTabBar.tsx@Split right;src/stores/workspaceLayoutStore.ts@addPaneToWorkspace|ユーザーとして現在のタブを右または下に分割して複数作業を並べたい。|PaneHeaderまたはPaneTabBarからsplit right/downを呼び、workspaceLayoutStoreが列/行構造に新規タブを追加する。|
MYC-009|タブ配置|タブクローズ|P0|Implemented per UI/store|No|src/components/workspace/PaneHeader.tsx@Close pane;src/components/workspace/PaneTabBar.tsx@Close pane;src/stores/workspaceLayoutStore.ts@removePaneFromWorkspace|ユーザーとして不要になったタブを閉じたい。|UIからClose paneを選ぶとremovePaneFromWorkspaceが対象タブをワークスペースから取り除く。|最後のタブ時の挙動を後続確認。
MYC-010|タブ配置|タブとペインのドラッグ移動|P1|Implemented per store|No|src/hooks/usePaneDragSource.ts@usePaneDragSource;src/stores/workspaceLayoutStore.ts@moveTabToPane;src/stores/workspaceLayoutStore.ts@movePaneToNewWorkspace;src/components/workspace/PaneDragOverlay.tsx@PaneDragOverlay|ユーザーとしてタブやペインを別ペイン、分割位置、新規ワークスペースへ移動したい。|drag storeとlayout storeがtab-to-pane、tab-to-split、pane-to-pane、pane-to-split、new-workspace移動を提供する。|実操作テストが必要。
MYC-011|ペイン|新規ターミナルペイン追加|P0|Implemented per UI/store|No|src/components/workspace/PaneTabBar.tsx@New terminal tab;src/stores/workspaceLayoutStore.ts@addTabToPane|ユーザーとして同じペイン内に新しいターミナルタブを追加したい。|PaneTabBarのNew terminal tabからaddTabToPaneがterminalタブを作成する。|
MYC-012|ペイン|ペイン切替とアクティブ管理|P0|Implemented per store/render|No|src/stores/workspaceLayoutStore.ts@setActivePaneTab;src/components/workspace/WorkspaceView.tsx@activeTab;src/components/workspace/TerminalPane.tsx@sessionId|ユーザーとしてペイン内のタブをクリックして表示対象を切り替えたい。|setActivePaneTabが対象ペインのactive_tab_idを更新し、WorkspaceViewとTerminalPaneが対応セッションを表示する。|
MYC-013|ペイン|ペイン名の編集|P0|Implemented per recent changelog/code|No|src/components/workspace/PaneTabBar.tsx@setTabLabel;src/stores/workspaceLayoutStore.ts@setTabLabel;CHANGELOG.md@ダブルクリック編集|ユーザーとしてターミナルペイン名を任意のラベルへ変更し、必要なら自動名に戻したい。|PaneTabBarがダブルクリックまたはメニューで編集状態に入り、setTabLabelでlabelを保存する。空またはリセットで自動名へ戻る。|直近修正領域。操作テスト優先。
MYC-014|ターミナル|PTYセッション作成|P0|Implemented per IPC/Rust|No|src/lib/ipc.ts@create_session;src/components/workspace/TerminalPane.tsx@<XTermWrapper;src-tauri/src/commands/terminal.rs@create_session;src-tauri/src/pty/manager.rs@create_session|ユーザーとして各ペインで選んだエージェントやシェルのターミナルを起動したい。|createSession IPCがagentId、cwd、session metadataを渡し、Rust PTY managerがセッションを作成する。|新規プロセス起動が絡む。
...
MYC-016|ターミナル|リサイズ同期|P0|Implemented per IPC/UI|No|src/lib/ipc.ts@resize_session;src/components/terminal/XTermWrapper.tsx@FitAddon;src-tauri/src/commands/terminal.rs@resize_session|ユーザーとしてタブサイズ変更後もターミナル表示を崩さず使いたい。|XTerm fit/resizeがcols/rowsを算出し、resize_session IPCでPTYサイズに反映する。|
...
MYC-020|エージェント|組み込みエージェント選択|P0|Implemented per lib/UI/store|No|src/lib/agents.ts@AgentDefinition;src/components/setup/AgentSelector.tsx@AgentSelector;src/stores/workspaceLayoutStore.ts@setTabAgentId|ユーザーとしてClaude Code、Codex、Hybridなどの作業エージェントを選んで起動したい。|agents libraryがAgentDefinitionを提供し、setup/pane UIがagentIdをペインやタブへ割り当てる。|
...
MYC-033|テーマ|テーマ切替|P1|Implemented per UI/store|No|src/components/theme/ThemePicker.tsx@setTheme;src/stores/themeStore.ts@setTheme;src/components/theme/themeDefinitions.ts@THEMES|ユーザーとして好みの配色テーマへ切り替えたい。|ThemePickerとthemeStoreがthemeIdを変更し、CSS変数とterminal colorsを更新する。|
...
MYC-036|テーマ|色の微調整とリセット|P2|Implemented per UI/store|No|src/components/theme/ThemeTweakPanel.tsx@選んだテーマの微調整;src/components/theme/ThemeTweakPanel.tsx@この色だけ戻す;src/stores/themeStore.ts@resetThemeTweaks|ユーザーとして主要色を部分的に調整し、必要なら個別または全体を戻したい。|ThemeTweakPanelがpresets、detail colors、clear、resetを提供し、themeStoreがtweak colorを管理する。|
...
MYC-040|通知|作業完了と承認待ちバッジ|P0|Implemented per UI/lib|No|src/lib/notificationStatus.ts@isShellProcess;src/components/layout/TabItem.tsx@Waiting for approval;src/components/workspace/PaneTabBar.tsx@Work done|ユーザーとして非アクティブなペインやワークスペースの作業完了、承認待ちを見落としたくない。|metadata/notificationStatusがprocess stateを判断し、TabItemとPaneTabBarにWaiting for approvalやWork doneを表示する。|
...
MYC-051|エラー処理|Reactエラーバウンダリ|P1|Implemented per UI|No|src/components/common/ErrorBoundary.tsx@Pane crashed;src/components/layout/ErrorBoundary.tsx@Something went wrong|ユーザーとしてひとつのタブや画面がクラッシュしてもアプリ全体を失いたくない。|common/layout ErrorBoundaryがタブまたは画面クラッシュ時にfallbackを表示する。|意図的クラッシュ注入は後続で判断。
...
MYC-054|入力/保留|Copy Mode、Broadcast Input、Drag-and-Drop|P2|Pending doc / not proven implemented|No|docs/features/pending/input-and-interaction.md@# Input & Interaction Enhancements|ユーザーとしてコピー専用モード、複数タブ同時入力、D&D操作を使いたい。|pending docsは要件を示すが、全機能の実装証拠は未確認。|後続でコード有無を追加調査。
