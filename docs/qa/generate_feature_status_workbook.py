...
MYC-021|エージェント|Claude/CodexセッションIDマッピング|P0|Implemented per App/IPC|No|src/App.tsx@applyAgentSessionMappings;src/lib/ipc.ts@read_agent_session_mappings;src-tauri/src/commands/terminal.rs@read_agent_session_mappings|ユーザーとして起動済みClaude/Codexセッションを復元、識別できる状態で使いたい。|read_agent_session_mappingsとpane metadataがagent kind/session idをワークスペース状態へ反映する。|
