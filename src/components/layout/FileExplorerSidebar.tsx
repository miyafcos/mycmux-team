...
function isPreviewableArtifactPath(path: string): boolean {
  ...
  return Boolean(match && PREVIEWABLE_ARTIFACT_EXTENSIONS.has(match[1].toLowerCase()));
}
...
function getFilePreviewTarget(): { workspaceId: string; paneId: string; sessionId: string } | null {
  ...
  if (activePaneId) {
    for (const pane of workspace.panes) {
      if (pane.sessionId !== activePaneId && !pane.tabs.some((tab) => tab.sessionId === activePaneId)) {
        continue;
      }
      const tab = terminalTabForPane(pane, activePaneId);
      if (tab) {
  ...
  for (const pane of workspace.panes) {
    const tab = terminalTabForPane(pane, null);
    if (tab) {
      return { workspaceId: workspace.id, paneId: pane.id, sessionId: tab.sessionId };
    }
  }
...
async function openFileInAppPreview(path: string): Promise<boolean> {
  if (!isPreviewableArtifactPath(path)) return false;

  const target = getFilePreviewTarget();
  if (!target) return false;

  try {
    const info = await previewArtifactUriForSessionV2(target.sessionId, path);
    useWorkspaceLayoutStore
      .getState()
      .openOrReloadHtmlPreviewPane(target.workspaceId, target.paneId, info);
    return true;
  } catch (err) {
    console.warn("[fileExplorer] in-app preview failed, falling back to default app:", err);
    return false;
  }
}
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: 4,
        padding: 4,
        boxShadow: "var(--cmux-shadow-menu)",
        minWidth: 200,
      }}
    >
      {pendingAction && (
      ...
      {!ctx.isDir && isPreviewableArtifactPath(ctx.path) && (
        <MenuItem icon={<ExternalLink size={12} />} label="外部で開く" onClick={handleOpenExternal} />
      )}
      {ctx.isDir && (
        <>
          <MenuItem
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
        ...
        <input
          ...
          style={{
            ...
            border: "1px solid color-mix(in srgb, var(--cmux-accent) 70%, var(--cmux-border))",
            borderRadius: 3,
            color: "var(--cmux-text)",
            fontSize: 11,
            fontFamily: "inherit",
  ...
  return (
    <div
      ...
      style={{
        ...rowStyle,
        paddingLeft: depth * INDENT_PX + 6,
        background: selected ? "color-mix(in srgb, var(--cmux-accent) 18%, transparent)" : "transparent",
        boxShadow: selected
          ? "inset 2px 0 0 var(--cmux-accent), inset 0 0 0 1px color-mix(in srgb, var(--cmux-accent) 22%, transparent)"
          : "none",
