...
export function DashboardDetailPane({ card, now, onJump }: {
  ...
  if (!card) return <aside style={paneStyle}><span style={{ color: "var(--cmux-text-secondary)", fontSize: "var(--cmux-font-size-sm)" }}>{dashboardStrings.detailEmpty}</span></aside>;
  ...
  return <aside style={paneStyle}>
    <div style={{ display: "flex", alignItems: "center", gap: 6, minWidth: 0 }}>
      <strong style={{ minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontSize: "var(--cmux-font-size-sm)" }}>{card.workspace.name} › {dashboardStrings.paneLocation(card.tabIndex + 1, card.paneIndex + 1)}</strong>
      <span style={statusStyle}>{groupLabel(card.group)}</span>
      <span style={{ marginLeft: "auto", color: "var(--cmux-text-tertiary)", fontSize: "var(--cmux-font-size-xs)" }}>{elapsed}</span>
    </div>
    <div style={{ display: "grid", gap: 6, marginTop: 12, fontSize: "var(--cmux-font-size-xs)", color: "var(--cmux-text-secondary)" }}>
    ...
    <div style={{ marginTop: 14, fontSize: "var(--cmux-font-size-xs)", color: "var(--cmux-text-secondary)" }}>{dashboardStrings.tailTitle}</div>
    <pre style={{ flex: 1, minHeight: 0, overflow: "auto", margin: "6px 0 0", padding: 10, border: "1px solid var(--cmux-border)", borderRadius: "var(--cmux-radius-sm)", background: "var(--cmux-bg)", color: "var(--cmux-text)", fontSize: "var(--cmux-font-size-xs)", whiteSpace: "pre-wrap", fontFamily: "monospace" }}>{unobserved ? dashboardStrings.unobservedBadge : lines.join("\n")}</pre>
  </aside>;
}
...
const statusStyle = { flex: "0 0 auto", border: "1px solid var(--cmux-border)", borderRadius: 999, color: "var(--cmux-text-secondary)", fontSize: "var(--cmux-font-size-xs)", padding: "1px 5px" };
