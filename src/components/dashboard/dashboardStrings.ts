...
export const dashboardStrings = {
  ...
  stateNotStarted: "未起動",
  notStartedDetail: "まだ開いていないペインです (端末を開くと記録が始まります)",
  // v2: 常設コンポーザ (ReplyComposer)
  composerPlaceholder: "ここに指示をタイプ",
  composerSend: "送信 ⏎",
  composerAriaLabel: "選択中のペインへの指示入力",
  composerNotStarted: "まだ開いていないペインなので送信できません",
  ...
  sendUnverifiedTargetUnmounted: "入力をキューに追加しましたが、対象ペインが未マウントのため画面で確認できませんでした",
...
export const tabGroupingStrings = {
  ...
  analysisBackground: "表示中の案で作業できます。AI案は作成を継続中",
  analysisSlowHint: "10秒を超えました。AI案を待たずに、表示中の案を編集・適用できます。",
  analysisSlowNoPlanHint: "10秒を超えています。AI案はまだ完成していません。",
  analysisDuration: (seconds: number): string => `AI案の作成: ${seconds}秒`,
  analysisTimingDetail: (scan: number, judge: number, validation: number): string =>
    `画面取得 ${scan}秒 / AI応答 ${judge}秒 / 検証 ${validation}秒`,
  judgeReadyAfterWait: "AI案ができました。「AI案に切り替える」で確認できます。",
  judgeReadyKeepingCurrent: "AI の案ができました。編集中の案を残しています。「AI案に切り替える」で確認できます。",
  judgeFailedKeepingCurrent: "AI の案を取得できませんでした。表示中の案は引き続き編集・確認できます。",
  showReadyPlans: "AI案に切り替える",
  showReadyPlansHint: "現在の編集内容を置き換えて、生成済みのAI案を表示します",
  analysisDetails: "応答の詳細を表示",
  partialPlans: "一部の案を使えませんでした。有効な案を表示しています。",
  localPlanWhileJudging: "表示中の案は作業フォルダと系譜から組んだものです。このまま編集・適用できます。",
  analyzeAgain: "再分析する",
  analyzed: "分析完了",
  unassignedTitle: "未分類",
  moveToUnassigned: "未分類へ",
  applyZeroMoves: "動かすペインがありません",
  warningsTitle: "警告",
  close: "閉じる",
  empty: "再配置できる生きたターミナルがありません",
  comparisonInsufficient: "案を1件作成しました。このまま編集・確認できます。",
  strategyProject: "案件",
  strategyRole: "役割",
  ...
  sideBySideLegend: "線は別のワークスペースへ移るペイン / 線の色は移動先のワークスペース / ペインをクリックすると1本だけ固定して追えます",
  ...
  lineageOrphan: "起動元のペインは残っていません",
  sideBySideGroupAriaLabel: (side: string): string => `${side}の配置図。矢印キーでペインを移動できます`,
  ...
  sideBySideNoMoves: "移動するペインはありません",
  apply: "適用",
  applying: "適用しています…",
  ...
  notDeleted: "削除されません",
  newBadge: "新規",
  undoApplied: (n: number): string => `再配置を適用しました — ${n}ペイン移動`,
  ...
  recallUndo: "直前の再配置",
  headCounts: (tabs: number, workspaces: number): string => `${tabs}ペイン / ${workspaces}ワークスペース`,
  planEditing: (title: string): string => `${title} を編集中`,
  toEdit: "この案を編集",
  confirmPlan: "この案で確認",
  editPlan: "内容を編集",
  changedOnly: "変更対象のみ表示",
  editMapLegend: "ペインを選んで、右の配置図の移動先タブをクリックします",
  keepTrayHint: "ここへ入れたペインは現在の位置に残ります",
  emptyGroupDropHint: "ペインをここへ戻すと、元の配置を復元します",
  ...
  stateKept: "現状維持",
  stateUnassigned: "未分類",
  editSummary: (moved: number, kept: number, unassigned: number) => `${moved}ペイン移動 / ${kept}ペイン現状維持 / ${unassigned}ペイン未分類`,
  ...
  backToEdit: "編集へ戻る",
  planMoveNote: (n: number): string => `この案で ${n}ペイン移動`,
  unassignedCount: (n: number): string => `未分類 ${n}`,
  previewLegend: "青=移動するペイン / 黄=未分類のペイン / 破線=新しいワークスペース",
  adoptSwitchLabel: (title: string): string => `${title}を採用する`,
  groupAdoptedMeta: (n: number): string => `新規ワークスペース・${n}ペイン`,
  destinationHeading: (name: string): string => `移動先: ${name}`,
  selectHint: "ペインをクリックで選択",
  columnLabel: (n: number): string => `列${n}`,
  emptyColumn: "（空）",
  fromWorkspace: (name: string): string => `${name}から`,
  excludeToCurrent: "除外して現状維持",
  confirmSummary: (moved: number, created: number, kept: number): string => `${moved}ペインを移動 / ${created}個のワークスペースを作成 / ${kept}ペインは現状維持`,
  confirmSummaryNote: "適用は1回で確定し、「元に戻す」で元の配置に戻せます。空になったワークスペースは削除されません。",
  undoDismissLabel: "閉じる",
  newGroupTitle: "新しいグループ",
  stepsAriaLabel: "手順",
  tabCount: (n: number): string => `${n}ペイン`,
  statusPoisoned: "ペイン再配置の復旧確認に失敗しました。安全のため、この機能を停止しました。現在のターミナルは引き続き使用できます。作業内容を確認してアプリを再起動してください。レイアウトは保存されません。再起動で最後の正常状態に戻ります。",
  statusCopyDiagnostics: "診断情報をコピー",
  statusInspectLayout: "現在の配置を確認",
  statusRestartApp: "アプリを再起動",
  statusUndoAvailable: "ペインの再配置を適用しました。",
  statusUndo: "ペインの再配置を元に戻す",
  statusDismiss: "×",
