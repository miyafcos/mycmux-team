...
// API field identifiers stay English; the table header speaks Japanese ($/Mtok).
const fieldLabels: Record<(typeof fields)[number], string> = {
  inputPerMtok: "入力",
  outputPerMtok: "出力",
  cacheReadPerMtok: "キャッシュ読み",
  cacheWrite5mPerMtok: "キャッシュ書き (5分)",
  cacheWrite1hPerMtok: "キャッシュ書き (1時間)",
};
...
export function PriceSettings({ prices, unpricedModels, loading, error, repricedSessions, onSave }: { prices: PriceEntry[] | null; unpricedModels: string[]; loading: boolean; error: string | null; repricedSessions: number | null; onSave: (entry: PriceEntry) => void | Promise<void> }) {
