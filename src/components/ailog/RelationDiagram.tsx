...
import { OTHER_KEY, buildProjectSankeyGraph, type SankeyNodeDatum } from "./sankeyModel";
import type { AilogSelection, TopNSetting } from "../../stores/ailogStore";
import { ScrollBox, noteStyle, subtleButtonStyle } from "./ui";
...
export function RelationDiagram({ models, sessions, excludeSynthetic, topN, onTopN, grandTotal, selection, onSelect }: {
  ...
  const visibleTags = selection?.type === "tag"
    ...
    : models.byWorkTag;
  const graph = useMemo(() => buildProjectSankeyGraph({
    byWorkTag: visibleTags, sessions: visibleSessions, totalSessions: sessions.total,
    excludeSynthetic, grandTotal, topN,
  }), [visibleTags, visibleSessions, sessions.total, excludeSynthetic, grandTotal, topN]);
  ...
  const selectionKey = selection ? `${selection.type === "project" ? 0 : selection.type === "tag" ? 1 : 2}:${selection.key}` : null;
  ...
  return <div style={{ display: "flex", flexDirection: "column", gap: 10, minWidth: 0 }}>
    <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
      ...
      {(["tag", "model"] as (keyof TopNSetting)[]).map((layer) => <label key={layer} style={{ fontSize: 10, color: "var(--cmux-text-tertiary)" }}>
        {layer === "tag" ? "作業種別" : "モデル"}
        <select aria-label={`${layer} の表示件数`} value={String(topN[layer])} onChange={(event) => onTopN(layer, Number(event.target.value))} style={selectStyle}>
    ...
    {empty ? <div style={noteStyle}>この期間に描ける関係がありません。</div> : <ScrollBox><svg viewBox={`0 0 ${VIEW_W} ${height}`} role="img" aria-label="案件から作業種別、モデルへのコストの流れ" style={{ width: "100%", minWidth: 900, height: "auto", display: "block" }}>
      {["案件", "作業種別", "モデル"].map((label, index) => <text key={label} x={COLUMN_X[index]} y={18} style={headerTextStyle}>{label}</text>)}
      ...
      {layout.nodes.map((node) => <NodeMark key={`${node.layer}:${node.key}`} node={node} datum={data.nodes.get(`${node.layer}:${node.key}`)} color={colors.get(`${node.layer}:${node.key}`) ?? NEUTRAL_COLOR} selected={selectionKey === `${node.layer}:${node.key}`} dimmed={selectionKey !== null && selectionKey !== `${node.layer}:${node.key}`} onClick={() => {
        ...
        const type = node.layer === 0 ? "project" : node.layer === 1 ? "tag" : "model";
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 3, ...noteStyle }}>
      <div>{`作業種別は重複します（期間の総額 ${formatUsd(graph.notes.grandTotal)}）。複数種別を持つセッションは按分していません。`}</div>
      ...
      {graph.notes.untaggedSessions > 0 ? <div>{`作業種別が付かなかったセッション ${formatCount(graph.notes.untaggedSessions)} 件（${formatUsd(graph.notes.untaggedCost)}）は関係図の中間層に出ていません。`}</div> : null}
    </div>
  </div>;
}
