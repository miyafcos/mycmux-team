...
const KIND_LABELS: Record<FindingKind, string> = {
  cause: "原因", constraint: "制約", gotcha: "ハマり", decision: "決定", verified: "検証済",
};

export function LearningView({
  findings, rankings, kind, query, loading, error, onKindChange, onQueryChange, onLoadMore, onOpenDetail,
}: {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      <Section title="学び">
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          <ButtonGroup options={KIND_OPTIONS} value={kind ?? "all"} onChange={(value) => onKindChange(value === "all" ? null : value)} ariaLabel="学び" />
        ...
        {error ? <EmptyState kind="error" message={error} /> : rows.length === 0 && !loading ? <div style={{ ...noteStyle, marginTop: 12 }}>該当する学びはまだありません (要約が進むと増えます)</div> : (
          <>
            <div style={{ display: "flex", flexDirection: "column", gap: 7, marginTop: 12 }}>
              {rows.map((row, index) => <button key={`${row.sessionKind}:${row.sessionId}:${index}`} type="button" onClick={() => onOpenDetail(row.sessionKind, row.sessionId)} style={{ textAlign: "left", border: "1px solid var(--cmux-border-hairline)", borderRadius: 6, background: "var(--cmux-hover)", color: "var(--cmux-text)", padding: "8px 10px", cursor: "pointer" }}>
                <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}><Chip tone="accent">{KIND_LABELS[row.kind]}</Chip>{row.repeatCount >= 2 ? <Chip tone="warn">同じ罠 {row.repeatCount} 回目</Chip> : null}<span style={noteStyle}>{new Date(row.date).toLocaleDateString("ja-JP")} · {row.projectLabel ?? "—"}</span></div>
                <div style={{ marginTop: 5, fontSize: "var(--cmux-font-size-sm)" }}>{row.text}</div>
              </button>)}
            </div>
            {findings && rows.length < findings.total ? <button type="button" onClick={onLoadMore} disabled={loading} style={{ ...subtleButtonStyle, alignSelf: "flex-start", marginTop: 10 }}>さらに読み込む</button> : null}
          </>
        )}
      </Section>
      <Section title="失敗コマンド上位">
