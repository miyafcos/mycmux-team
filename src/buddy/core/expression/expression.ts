...
import type { BuddyReaction, BuddyViewModel, JudgmentDecision, UtteranceLogEntry } from "../types";
...
export class ExpressionController {
  ...
  constructor(
    private readonly onChange: (state: BuddyViewModel) => void,
  ...
  flashMood(
    mood: BuddyViewModel["mood"],
    durationMs: number,
    options: { bypassSilent?: boolean } = {},
  ): void {
    if (this.state.loading || (this.isSilent() && !options.bypassSilent)) {
      return;
    }
    if (this.flashTimerId !== null) {
      this.scheduler.clearTimeout(this.flashTimerId);
      this.flashTimerId = null;
    }
    this.state = { ...this.state, mood, reaction: null };
    this.commit();
    this.flashTimerId = this.scheduler.setTimeout(() => {
      this.flashTimerId = null;
      ...
      this.state = { ...this.state, mood: nextMood, reaction: null };
      this.commit();
    }, durationMs);
  }

  flashReaction(reaction: BuddyReaction): void {
    if (this.isSilent()) {
      return;
    }
    if (this.flashTimerId !== null) {
      this.scheduler.clearTimeout(this.flashTimerId);
      this.flashTimerId = null;
      if (!this.state.reaction && !this.state.loading) {
        const nextMood = this.state.speech ? this.state.speech.mood : "idle";
    ...
    this.flashTimerId = this.scheduler.setTimeout(() => {
      this.flashTimerId = null;
      this.state = { ...this.state, reaction: null };
      this.commit();
    }, reaction.durationMs);
  }
  ...
  private scheduleIdleDecay(): void {
    ...
    this.idleDecayTimerId = this.scheduler.setTimeout(() => {
  ...
  private clearIdleDecay(): void {
    if (this.idleDecayTimerId !== null) {
      this.scheduler.clearTimeout(this.idleDecayTimerId);
      this.idleDecayTimerId = null;
    }
  }
