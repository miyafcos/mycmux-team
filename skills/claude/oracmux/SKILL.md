...
## 送信は「入れる」と「入った」を分ける (2026-09-10 の実害と対策)

宮崎さんの初回利用が**質問を送る手前で止まった**。ChatGPT に 7,283 字の引き継ぎ書を `web.push` で入れたところ、
文字は画面に見えているのに送信ボタンが出ず `pane: ChatGPT submit button was not found` で終わった。

- **原因**: ChatGPT の入力欄は ProseMirror で、自分が作っていない DOM を渡されると取り込みを拒むことがある。
  `web.push` は要素へ文字を代入して `input` を 1 発撃つだけなので、拒まれると**画面には出たまま editor の中身は空**になる。
  中身が空なら送信ボタンは描画されない。**長さは無関係** (17,000 字の素のテキストは通る)。
  引き金は**末尾のコードフェンス** — oracmux の引き継ぎ書は出力契約を ``` で閉じるので、毎回この形になる
- **対策**: 入れる工程と送る工程を分けた。`web.push` の後に**送信ボタンが出たかどうか**で「editor が受け取った」ことを確かめ、
  出なければ `web.type` (beforeinput + execCommand → CDP insertText) で打ち直す。それでも出なければ **1 ターンも使わず exit 3** で止める。
  `ask` も `push` も同じ経路を通る (`push` は送らないので、宮崎さんが手で送れる状態まで面倒を見る)
- **検知**: `doctor --deep` が毎回、末尾フェンス付きの本物と同じ形を入れて送信ボタンの出現を確かめる (0 ターン・入れた後は消す)。
  **セレクタが全部合っていても composer に入らない**ことがあると分かったので、セレクタ照合だけでは足りない
