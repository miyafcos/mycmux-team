...
def build_parser() -> argparse.ArgumentParser:
    ...
    doctor.add_argument("--deep", action="store_true", help="engines.json のセレクタを実 DOM と突き合わせ、モデル picker のラベルも読む (裏ペインを開く・Web ターンは消費しない)")
    ...
    smoke.add_argument("--keep-tabs", action="store_true", help="(既定と同じ) ペインを残す。将来の自動クローズ用に予約")
    ...
    ask.add_argument("--via", choices=("pane", "oracle", "cdp"), help="経路: pane = mycmux の 裏の Web ペイン (既定・全エンジン) / oracle = steipete oracle CLI (chatgpt・OracleChrome) / cdp = 自前ドライバ (OracleChrome)")
    ask.add_argument("--tab", help="pane 経路: 既存の Web ペイン (web-list の tabId) を使う (既定は裏ペインを新規に開く)")
    ask.add_argument("--close-tab", action="store_true", help="pane 経路: 回収後にペインを閉じる (既定は残す)")
    ...
    ask.add_argument("--url", help="この URL でペインを開く (ChatGPT のプロジェクト/フォルダなど)")
    ...
    followup.add_argument("--tab", help="続ける会話のペイン (既定: そのサービスの最新の裏ペイン)")
    ...
    followup.add_argument("--close-tab", action="store_true", help="回収後にペインを閉じる")
    ...
    council.add_argument("--close-tabs", action="store_true", help="pane 経路: 回収後にペインを閉じる")
    ...
    target.add_argument("--url", help="会話 URL を裏ペインで開いて回収 (pane) / OracleChrome で開いて回収 (cdp)")
    target.add_argument("--tab", help="pane 経路: 読む Web ペインの tabId (web-list)")
    ...
    collect.add_argument("--close-tab", action="store_true", help="pane 経路で --url から開いたペインを回収後に閉じる")
