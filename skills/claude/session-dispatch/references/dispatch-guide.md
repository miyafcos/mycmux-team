...
- `status` 語彙は `dispatch_ledger.py` の定数が正本:
  `open` / `running` (作業中) → `done` (DONE.md 自己申告・ペインは生存) →
  `closed` / `done-verified-closed` (ペイン片付け済) / `abandoned` / `fallback-inline` (異常終了)。
  close 失敗は `close_failed` (ペイン未回収・再検収対象) として別記する。
  ペイン片付け済・異常終了の4つが **クローズ集合** = 二度と操作してはいけない集合 (watcher も hook もこれで fail-closed 判定)
...
spawn の stdout は mycmux socket の応答 JSON。`sessionId` (ペインの pty セッション ID) が返る —
...
- 返答が来ない・壊れている場合はペインを目視 (可視ペインなので画面で確認できる) → 手動判断
...
- ペインを放置しない — mycmux はアイドル 120 分で自動 dormant になるが、閉じるのが正
