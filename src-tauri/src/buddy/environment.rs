...
fn parse_skill_inventory(raw: &str) -> Vec<(String, SkillEntry)> {
    ...
    let flush =
        |name: &mut Option<String>, entry: &mut SkillEntry, out: &mut Vec<(String, SkillEntry)>| {
...
fn build_rules_block(dir: &Path) -> Option<String> {
    ...
    for path in files.iter() {
        ...
        let name = path.file_stem().and_then(|n| n.to_str()).unwrap_or("rule");
...
fn build_claude_md_block(home: &Path, claude_dir: &Path) -> Option<String> {
    let candidates = [claude_dir.join("CLAUDE.md"), home.join("CLAUDE.md")];
