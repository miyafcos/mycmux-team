...
pub fn internal_origin(conn: &Connection) -> Result<(), String> {
    ...
    conn.execute("UPDATE session SET origin='ailog-internal' WHERE kind='codex' AND first_prompt LIKE '[mycmux-ailog-summarizer]%' AND COALESCE(origin,'unknown') <> 'ailog-internal'", []).map_err(|err| format!("mark internal Codex sessions: {err}"))?;
