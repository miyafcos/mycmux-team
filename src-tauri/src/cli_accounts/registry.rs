...
#[derive(Serialize, Deserialize, Default, Clone)] pub struct ActivePointers { #[serde(default)] pub claude: Option<String>, #[serde(default)] pub codex: Option<String> }
...
pub fn remove_profile(file: &mut CliAccountsFile, id: &str) -> bool { let before=file.profiles.len(); file.profiles.retain(|p|p.id!=id); if file.active.claude.as_deref()==Some(id){file.active.claude=None;} if file.active.codex.as_deref()==Some(id){file.active.codex=None;} before != file.profiles.len() }
...
pub fn set_active(file: &mut CliAccountsFile, provider: CliProvider, id: Option<String>) { match provider { CliProvider::Claude=>file.active.claude=id, CliProvider::Codex=>file.active.codex=id } }
