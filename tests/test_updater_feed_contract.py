...
Two invariants that used to live only in a human's memory are pinned here.
...
1. Normalization (0.9.x dual-install split-brain): the plain "windows-x86_64"
   ...
   place that enforces this for both the master and lite updater feeds.
...
# Only the PowerShell-driven normalization tests need Windows; the key-id tests
# below are pure Python and must run everywhere.
...
def all_windows_keys(key_id: str) -> dict[str, str]:
    return {
...
def test_verify_feed_passes_when_every_platform_uses_the_expected_key() -> None:
    feed = make_feed("9.9.9", all_windows_keys(CURRENT_KEY_ID))
...
def test_verify_feed_fails_when_signed_with_a_rotated_away_key() -> None:
    ...
    feed = make_feed("9.9.9", all_windows_keys(ROTATED_AWAY_KEY_ID))
...
def test_verify_feed_fails_when_only_one_platform_entry_is_mis_signed() -> None:
    key_ids = all_windows_keys(CURRENT_KEY_ID)
...
def test_verify_feed_fails_on_a_stale_feed_version() -> None:
    ...
    feed = make_feed("9.9.8", all_windows_keys(CURRENT_KEY_ID))
    assert verify_updater_feed.verify_feed(feed, "9.9.9", CURRENT_KEY_ID) is False
...
def test_verify_cli_offline_exits_zero_on_a_good_feed(tmp_path: Path) -> None:
    ...
    feed_path = write_feed(tmp_path, make_feed("9.9.9", all_windows_keys(config_key_id)))
...
def test_verify_cli_offline_exits_nonzero_on_a_mis_signed_feed(tmp_path: Path) -> None:
    feed_path = write_feed(tmp_path, make_feed("9.9.9", all_windows_keys(ROTATED_AWAY_KEY_ID)))
