...
mycmux は起動時に 127.0.0.1 のランダムポートで待ち受け、ポート番号を `~/.mycmux/mycmux.port` に書き出します。同梱の CLI (`scripts/mycmux_agent_cli.py`、Python 標準ライブラリのみで動作) がこのポートに JSON を1行送り、mycmux 側がペイン操作を実行して結果の JSON を返します。待ち受けはループバック限定で、外部ネットワークからは接続できません。さらに全リクエストには起動ごとに生成されるトークン (`~/.mycmux/mycmux.token`) が必要で、同じ PC 上の無関係なプロセスからは操作できません (同梱 CLI は自動で添付します)。
