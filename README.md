...
mycmux は、Claude Code / Codex / claude-codex / Grok Build といった AI エージェント CLI を、ワークスペース・タブ・ペインの単位で並べて動かすターミナルワークスペースです。Windows と macOS (Apple Silicon) で動き、エージェントが出したパスをワンクリックで開く、作業の区切りを保存して人へ渡す、複数アカウントの使用量をタイトルバーで見張る、エージェント自身が可視ペインで別のエージェントを起動する、といった実務向けの機能を足しています。[cmux-for-linux (ptrcode)](https://github.com/cai0baa/cmux-for-linux) のフォークで、ライセンスは GPL-3.0 です。配布物は公開ミラーの固定 feed リリース ([mycmux-personal-updater](https://github.com/miyafcos/mycmux-team/releases/tag/mycmux-personal-updater)) から取得します。現行版は v0.64.0 (2026-09-06) です。
...
参照部: [4. ワークスペース・タブ・ペイン](#4-ワークスペースタブペイン) / [5. ターミナル](#5-ターミナル) / [6. エージェントを起動する](#6-エージェントを起動する) / [7. エージェントに委譲する](#7-エージェントに委譲する) / [8. セッションを残す・戻す](#8-セッションを残す戻す) / [9. ダッシュボード](#9-ダッシュボード) / [10. 成果物を見る・直す](#10-成果物を見る直す) / [11. アカウントと使用量](#11-アカウントと使用量) / [12. AI ログ分析 (ailog)](#12-ai-ログ分析-ailog) / [13. 通知と状態検知](#13-通知と状態検知) / [14. 見た目と設定](#14-見た目と設定) / [15. スマホ・Buddy・ペット](#15-スマホbuddyペット)
...
| 番号 | 場所 | できること |
| --- | --- | --- |
| 1 | Workspaces (サイドバー) | 案件、調査、実装などの単位で作業場所を切り替える。行にはキャラ、承認待ちバッジ、ペイン数、ペイン名が出る |
| 2 | Split panes | Codex、Claude Code、ログ確認などを横に並べて見る。1 タブの中はさらにペインで分かれる |
| 3 | Active pane | 今操作しているタブを見分け、`Ctrl+Shift+Enter` で拡大する |
| 4 | Review / notes | 差分、メモ、別エージェントの出力、成果物プレビューを同時に確認する |
| 5 | Theme background | テーマ 30 本と壁紙 59 枚を切り替えて、長時間作業しやすくする |

タイトルバーは左からサイドバー開閉・セーブポイント・新規ワークスペース、中央にワークスペース名、右に使用量メーター・AI ログ分析・ダッシュボード・設定が並びます。ボタンの全一覧は [4 章](#タイトルバーのボタン)にあります。
...
- ペインの中のエージェントに「続きは Codex で」と頼むと、エージェント自身が引き継ぎ書つきの可視ペインを開いて相手を起動する
- ダッシュボード (`Ctrl+Shift+G`) で全ワークスペースを俯瞰し、会話を最大 5 列並べ、`@` で宛先を指定してまとめて指示を送る
- Claude の質問 (AskUserQuestion) を別ペインへ行かずにカードで読み、選択肢ボタンで答える
...
- 誤って閉じたペインは `Ctrl+Shift+T` で会話ごと復活し、アプリを再起動しても前回のタブ構成と会話へ戻る
...
**日本語の表示にフォントの用意は要りません。** ターミナルの書体 (UDEV Gothic NF) はアプリに
同梱しています。macOS には日本語の等幅フォントが標準では入っておらず、同梱しないと英数字が
Menlo、日本語がヒラギノに分かれます。この 2 つは文字幅が噛み合わないため、罫線や表が 1 文字ごとに
ずれていきます。設定 → 表示 で別の書体に変えられますが、その環境に存在しない書体には
「この環境に無し」と出ます。保存されていた設定がその Mac で表示できない書体だった場合は、
起動時に同梱の書体へ切り替えたことをお知らせします。
...
**システム許可を毎回聞かれる問題は直しました。** 原因は署名です。ビルドしたままの `.app` は
リンカが付けたアドホック署名しか持たず、証明書を含みません。証明書が無いと macOS は許可を
CDHash (バイナリのハッシュ値) に紐づけるので、ビルドし直すたびに別のアプリと見なされ、
デスクトップ・書類・ダウンロード・ネットワークボリュームの許可が失効して聞き直されていました。

`scripts/build-mac.sh` がローカルの署名証明書を自動で作って署名します。ソースからビルドする
場合、証明書まわりで用意するものはありません。初回のビルドで
`~/Library/Keychains/mycmux-signing.keychain-db` に証明書が作られ、以後は同じものが使われます。
Apple Developer Program も Xcode も要りません。署名の状態は次で確認できます。

```bash
bash scripts/mac-local-signing-identity.sh status
codesign -d -r- /Applications/mycmux.app   # designated => ... certificate root = H"..."
```

`certificate root` が出れば証明書方式です。`cdhash` が出る場合はアドホック署名のままなので、
更新のたびに許可を聞かれます。

Apple の Developer ID を持っている場合は環境変数で渡してください。そちらが優先され、
Gatekeeper の警告も出なくなります。

```bash
APPLE_SIGNING_IDENTITY="Developer ID Application: ..." bash scripts/build-mac.sh
```
...
**2. ランチャーで Claude Code を立てる** — 新しいタブを開くと、そのペイン自体が起動メニュー (ランチャーペイン) になります。検索欄にフォーカスが入った状態なので、`Claude Code` の行を `↓` で選んで `Enter` を押すか、`cl` と打って絞り込んでから `Enter` を押すと起動します。先に作業フォルダを変えたいときは、下の「開発」「案件」の一覧でフォルダの行を選んでから (フッタに起動先が出ます) 項目を選びます。フォルダを変えてから起動すると、そのリポジトリの CLAUDE.md とプロジェクト履歴を持つセッションになります。

**3. 分割してもう 1 本立てる** — `Ctrl+Alt+D` で右に分割すると、新しいタブにも同じランチャーが出るので、`Codex` の行を選んで起動します。分割は「並べて見比べる」ためのもので、1 つのタブの中で切り替えたいだけならペインバーの「＋」でペインを足すほうが場所を食いません。

**4. パスリンクで成果物を開く** — エージェントが出力したファイルパスをクリックします。md / html / Office ならその場でプレビューペインが開き、それ以外はクリック位置に「既定のアプリで開く」「エクスプローラーで表示」の 2 アイコンが出ます。パスは実在チェックを通ったものだけがリンクになるので、存在しない文字列を押して空振りすることはありません。

**5. セーブポイントを残す** — 区切りがついたらペインバーの「セーブポイントを作成」ボタン (しおりのアイコン) を押します。サマリ 1 行 (空欄なら自動生成) を確認して保存すると、会話履歴の要約・引き継ぎ書・復元情報が 1 件のセーブポイントになります。同じセッションで押し直せば上書き更新、「終了時の記録を残す」を選べばあとから変わらない記録になります。中断のたびにこれを押しておくと、翌日は要点から再開できます。
...
用語は 3 段です。**ワークスペース**が案件などの作業場、その中に**タブ**がグリッドで並び、1 タブの中に**ペイン**が積まれます (用語集は[付録 H](#付録-h-用語集))。
...
`Ctrl+Shift+N` で作成し、`Ctrl+Tab` / `Ctrl+Shift+Tab` で前後へ、`Ctrl+1` `Ctrl+2` `Ctrl+3` `Ctrl+4` `Ctrl+5` `Ctrl+6` `Ctrl+7` `Ctrl+8` で番号ジャンプ、`Ctrl+9` で最後のワークスペースへ移動します。閉じるのは `Ctrl+Shift+W` で、中に生きているペインがあれば確認ダイアログが出ます。作成時のレイアウトは `1x1` `2x1` `3x1` `4x1` `2x2` `3x2` `1x2` `2x3` の 8 種から選べます (既定は `3x1`)。行の並べ替えはドラッグ、色分けは行ホバーで出る「⋮」→「色」から 8 色 +「なし」、名前変更はダブルクリックか「⋮」→「名前を変更」です。切り替えて戻ったときのフォーカスは、ワークスペースごとに最後にいたタブへ復元されます。

状態: ワークスペース本体 配信済み v0.1.0 / 色分け v0.21.31 (⋮ 化 v0.21.34) / フォーカス記憶 v0.9.0 / ようこそ画面 v0.21.30 / サイドバー幅のドラッグ変更 v0.50.0 (200〜560px・既定 280px)。

### 並べて見比べたい → タブを分割する

`Ctrl+Alt+D` で右、`Ctrl+Alt+Shift+D` で下に分割します。フォーカスは `Ctrl+Alt+ArrowLeft` `Ctrl+Alt+ArrowRight` `Ctrl+Alt+ArrowUp` `Ctrl+Alt+ArrowDown` (`Ctrl+Alt+←` `Ctrl+Alt+→` `Ctrl+Alt+↑` `Ctrl+Alt+↓`) で移し、`Ctrl+Alt+W` で閉じます。タブの境界はドラッグで比率を変えられ、比率はワークスペースに保存されます。1 つに集中したいときは `Ctrl+Shift+Enter` で拡大・復元します。タブを閉じる前には、裏で動いているエージェントペインを列挙した確認ダイアログが出ます。確認中に増えたペインも再チェックするので、長時間の委譲を巻き添えで殺しにくくなっています。最後の 1 タブは閉じられません。

状態: 分割 配信済み v0.1.0 / 「下に分割」ボタンの表示切替 v0.9.2・「右に分割」ボタンの表示切替 v0.9.5 (どちらも設定「通知とレイアウト」) / 巻き添え確認 v0.36.0。

### タブ内ペイン — 1 つのタブに端末を積んで切り替える

ペインバーの「＋」(`新しいターミナルペイン`) で追加し、`Ctrl+Alt+PageDown` / `Ctrl+Alt+PageUp` で巡回します。非アクティブのペインも PTY は動き続け、選んだときにスクロールバックを復元します。名前はダブルクリックで編集、空にすると自動命名へ戻ります。`Ctrl+Alt+P` でアクティブペインをピン留めするとペイン帯の先頭に固定されます。閉じるのはペインのつまみの「×」か中クリックです。

| したいこと | 入口 | 状態 |
| --- | --- | --- |
| 全ペインを 1 リストで見る | ペインバーの「⋯」(`ペイン一覧`)。狭いときは `1/3 ▾` 表記 | 配信済み v0.13.2 / 表示崩れの修正 v0.20.0 |
| 要対応のペインへ順に飛ぶ | `Ctrl+Alt+A` | 配信済み (初出版不明) |
| 閉じたペイン・タブを会話ごと戻す | `Ctrl+Shift+T` | 配信済み v0.11.0 / ペインの × も対象 v0.21.31 |
| 同じ会話から枝分かれさせる | ペインのつまみの「セッションを複製」ボタン | 配信済み v0.37.0 相当 |
| ダッシュボードで開く | ケバブメニュー「Open in dashboard」 | 配信済み (初出版不明) |
| タブ幅に応じてペインバーを畳む | 自動。`full → slim → compact → compact3 → compact2 → compact1 → micro` の 7 段階 | 配信済み (v0.15.0 の 2 段 → v0.19.0 で畳む順を再設計 → v0.36.0 でチップ化) |
| 溢れた操作をまとめて出す | 「⋮」(`タブの操作`) | 配信済み v0.16.0 |
| 非アクティブペインの名前を確かめる | 26px チップに 120ms ホバー | 配信済み v0.36.0 |

### 散らばったペインを組み替えたい → ドラッグ＆ドロップ

ペインのつまみまたはペインバーの余白を掴むと、カーソル追従のゴーストとドロップ先のプレビュー枠が出ます。落とせる先は「このタブのペインに追加」「タブを統合」「左にタブ」「右にタブ」「上にタブ」「下にタブ」「新しいワークスペースへ移動」です。相手のペインに生きたエージェントがいるときは、移動ではなく「◯◯ へ引き継ぎ文書を渡す」チップが出ます ([8 章](#8-セッションを残す戻す)参照)。状態: 配信済み (ラベル日本語化 v0.17.0 / 自タブへの無意味なチップを非表示 v0.19.0)。
...
### 「作業中」「入力待ち」はペインのドットで分かる

ペインと下部ステータスに「作業中」「入力待ち」を日本語ラベルと状態ドットで出します。作業中のドットは脈動します。判定は画面に `esc to interrupt` が出ているか、直近 7 秒に PTY 出力があるかで、静かになったエージェントは無表示になります。未確認の注意は「未確認の枠切れ」「未確認の入力待ち」「未確認のエラー」として区別されます。種別バッジは `Claude` / `Codex` / `Hybrid` / `Grok` / `Antigravity` です。状態: 配信済み v0.14.6 (日本語化) / 判定条件の変更 v0.21.11 / ペインバーへの集約 v0.24.0。
...
| HTML をサイドペインで見せる | エージェント側から `OSC 9988` を出力 | 配信済み (初出版不明) |
...
「Web」セクションの 5 行 (ChatGPT / Gemini / Grok / Claude.ai / NotebookLM) はプロセスではありません。選ぶとそのペイン自体が指定サービスの Web 画面になります (他の項目がシェルをそのプログラムに置き換えるのと同じ挙動です)。中身は Tauri の子 webview で、ペインの矩形に追従し、mycmux のグローバルショートカットは webview にフォーカスがあっても効きます。
...
Google は埋め込み webview からの OAuth を仕様として拒むことがあります。その画面を掴むとペイン上部に「ログインしていません」と**別の窓でログイン**ボタンが出ます。押すと、そのプロファイルフォルダを直接使って実 Edge の窓が開きます (WebView2 は Edge そのものなので、プロファイル形式も Cookie の暗号鍵も共通です)。ログインしてその窓を閉じると、ペインが自動で開き直ります。**この操作は 1 回だけ**で、以降はログイン済みで開きます。バーはログイン済みのときは出ません。
...
### 別のフォルダで立てたい → 「開発」「案件」の一覧

ランチャーペインの **開発** と **案件** の 2 つの一覧が起動先の候補です。行を押すと起動先がそのフォルダに変わり (フッタに表示)、そのあとに選んだ項目はそのフォルダで起動します。一覧の中身はあなたの環境ごとに育てます。置き場は `~/.mycmux/launch-dirs.json` (macOS も同じ) で、**設定 → ランチャー** から編集します。

- **登録する** — 「フォルダを選んで登録…」でフォルダを 1 つ選ぶと、その場で手動の行として入ります。表示名はクリックで編集、手動の行は ↑↓ で並べ替え、「削除」で外します。セクション名 (開発 / 案件) もクリックで変えられます (「Repos」「Clients」など)。
- **候補から拾う** — 「候補」ブロックには、ルールが見つけたフォルダ・エージェントを動かしたフォルダ・最近選んだフォルダのうち未登録のものが並びます。「開発に登録」「案件に登録」で入れ、「無視」で以後出さなくします。
...
「この続きの実装は Codex にやらせたい」というとき、これまではセッション要約を人間がコピーして貼り直すか、エージェントが裏で別プロセスを起動して見えないまま作業させるかの二択でした。mycmux ではエージェント自身が引き継ぎ書を書き、**目に見えるペイン**を開いて相手を起動します。作業は画面上のペインとして動くので途中経過を目で追えますし、そのペインをいつでも人間が直接引き継げます。mycmux 側はエージェントの振る舞いを強制しません。「委譲は見える形で行う」という方針は各エージェント側のルールファイルに書くことで成立します。契約の正本は [docs/agent-integration.md](docs/agent-integration.md) です。
...
| `spawn` | `pane.spawn_tab` / `pane.spawn` | エージェントを立ち上げる (既定は呼び出し元タブの裏ペイン) |
| `spawn-tab` | `pane.spawn_tab` | ペイン起動の低レベル版。`--` 以降に任意コマンドの argv を渡せる |
| `declare-tab` | `pane.declare_tab` | PTY を起こさずペインだけ宣言する |
| `activate-tab` | `pane.activate_tab` | ペインを内部的にアクティブ化する (操作者の前面は変えない) |
| `restore-activation` | `pane.restore_activation` | 直前の activation をトークンで検証・復元する |
| `close-tab` | `pane.close_tab` | ペインを閉じる |
| `rename` | `pane.rename_tab` | ペイン名を変える |
| `move` | `pane.move` | タブをワークスペース内で移動する |
...
既定の配置は**呼び出し元タブの新しいペイン**で、アクティブなペインは移動しません (`MYCMUX_PANE_SESSION_ID` から自動判定)。`--split` を付けたときだけ従来のタブ分割になり、`--workspace` / `--anchor-pane` / `--direction` は `--split` とセットです。応答 JSON の `placement` に `tab` / `pane` のどちらになったかが入ります。`MYCMUX_PANE_SESSION_ID` が無いペイン外からの実行はエラーで止まります (2026-08-21 以降、暗黙に新タブへは落ちません)。`--activate` は「操作者の前面を変えずに内部的なアクティブ化だけ許す」オプションで、バックグラウンドのワークスペースでのみ効きます。
...
`pane.read` の読み方は表示中かどうかで変わります。表示中のペインはレンダラのバッファを、背面ペインは PTY のスクロールバックをヘッドレス xterm で再生して読みます (最大 400 行)。古い文書にある「一度も表示していないペインは空が返る」は v0.14 当時の挙動で、現行はヘッドレス再生に置き換わっています (`socketCommands.ts` の `getHeadlessBufferLines`)。それでも裏ペインの完了検知は画面でなく上の 3 層で行ってください。
...
`MYCMUX_TERM_PROGRAM=mycmux` が「mycmux 内にいるか」の判定キーです。`MYCMUX_TAB_ID` / `MYCMUX_PANE_SESSION_ID` が自ペイン・自ペインの識別、`MYCMUX_MARKDOWN_OUT` / `MYCMUX_HTML_OUT` / `MYCMUX_ARTIFACTS_DIR` がセッション成果物の出力先です。全一覧は[付録 F](#付録-f-環境変数-mycmux_)にあります。
...
再起動すると、ワークスペース・タブ・ペイン・作業ディレクトリ・直前のターミナル表示に加えて、ウィンドウの位置・サイズ・最大化状態も復元します。Claude Code / Codex / claude-codex / Grok Build の前回セッションは、直前の会話表示と作業場所がペインに残ります。状態: 配信済み v0.5.6 (v0.4.0 で env 汚染事故により一時廃止 → 多層安全弁とセットに再導入) / 自己修復 v0.14.0 / handoff 起動でも決定的な ID を採番 v0.30.1。**v0.57.0 の残課題 #7「再起動時のペイン復活が不安定」は未解決**です ([16 章](#16-開発中の機能))。

### 別ペインの会話を横取りされたくない → 取り違えの防止

どのペインがどの会話を持つかは、「一番新しいファイル」からの推測ではなく記録ベースで決めます。対応表のパースは fail closed で、`<既知の kind>:<id>` の形だけを対応と見なします。同一フォルダで複数ペインを開いたときの ID 交差、壊れたセッション ID の保存、孤児ペインの残留を防ぎ、他ペインが既に持つ ID は候補から除外します。復元検証に失敗したときは `MYCMUX_SESSION_ID` と `MYCMUX_RESUME` を env から取り除き、警告トーストを出します。状態: 配信済み v0.8.17 / cross-contamination の修正 v0.13.4 / 壊れた ID の自動浄化と fail-closed 化 v0.30.1 / 複製・再起動での取り違えを根治 v0.36.0。

### Resume パレット (`Ctrl+P`) — 過去セッションを選んで開き直す

Claude Code / Codex / claude-codex の過去セッションを一覧し、選んで右分割タブに resume します。別種別を選べば引き継ぎ起動になります。ラベル・プレビュー・cwd・ID・種別をあいまい検索 (Fuse.js) でき、並び順は最終活動時刻のままです。cwd フィルタのチップは今アクティブなペインの cwd が先頭に来て初回は自動適用されます。初期 1000 件から「さらに過去のセッションを読み込む」で全件 (最大 10000 件) へ広げられます。実体ファイルが無く要約だけの Claude 履歴は、1 回目の `Enter` で警告し 2 回目で新セッションとして再開します。パレットの先頭には「操作 · ペイン掃除を開く」(`Ctrl+Shift+K`) も固定で出ます。
...
ペインのつまみの「セッションを複製」を押すと、会話記録を新しい ID に複製してから再開します (元のペインは動いたまま)。複製に失敗したときは引き継ぎ方式にフォールバックし、その旨をトーストで出します。状態: 配信済み v0.37.0 相当。誤って閉じたペイン・タブは `Ctrl+Shift+T` でセッションごと開き直せます。ペインの「×」、ワークスペースの一括クローズ (直近 5 ペインまで)、ペイン掃除で閉じたペインも対象です。状態: 配信済み v0.11.0 / 対象の拡大 v0.21.31。

### 作業の区切りを残したい → セーブポイント

Claude Code / Codex セッションの会話履歴から、要約 + 引き継ぎ書 (handoff.md) + 復元情報のバンドルを作ってこの PC のローカル領域へ保存します。共有フォルダ、期限付きリンク、受信コードは使いません。

**保存する側**の導線は 3 つです。ペインバーのしおりボタン (`セーブポイントを作成` / `セーブポイントを更新`)、ケバブメニューの「Publish savepoint…」「Update savepoint…」、セーブポイント画面の「このPCの作業」から各行の「ここまでを保存」。進捗は「会話履歴を読み取り・要約 → 引き継ぎ書 (handoff.md) を生成 → 引き継ぎ記録を作成 → 一覧へ登録」の 4 ステップで表示されます。**作業途中**は同じセッションの記録を上書き更新し、**終了時**は変更しない区切りの記録として残します。会話履歴があれば未起動のペイン (復元候補) からも保存できます。
...
全ワークスペースの列・タブ・ペインの実配置を縮小図で表示します。ワークスペース見出しにはペイン本数・返答待ち本数と、ペイン 1 本 = 1 目盛の帯 (12 本超は `+N`) が出ます。1 ペイン = 1 チップで、エージェント種別 (未起動なら「宣言のみ」)・状態ドット・「返答待ち」・CTX%・最終出力からの経過を表示します。クリックで列を開き、ダブルクリックで元の画面へ、`Alt+クリック` で同一グループを一括選択します。ドラッグの矩形選択・`Ctrl` / `Shift` クリック・`Alt` グループ選択で複数ペインを選び、「選択を閉じる」でまとめて閉じられます (確認ダイアログで対象名を列挙します)。フッターの「＋ 新しいワークスペース」で選択ペインを新規ワークスペースへ移せます。中央との境界はドラッグかキー (`←` `→` `Home` `End`) で幅を変えられます (最小 240px)。

状態: 配信済み v0.39.0 / 全ワークスペース展開・畳み状態の記憶・幅変更 v0.40.0 / 全ペイン表示とドラッグ選択 v0.46.2 / フラットな設計図として再描画 v0.54.0。

### 増えたペインを片づけたい → ペイン掃除

配置図フッターの「ペイン掃除」ボタン、`Ctrl+Shift+K`、コマンドパレットの「操作 · ペイン掃除を開く」から実行します。ワンクリック版は全ペインを走査して、終了済みペインと AI が「完了して待機中」と判定したペインをまとめて閉じ、結果をトースト (「N件のペインを閉じました」+ ボタン「取り消し」「内訳」) で返します。「内訳」または `Ctrl+Shift+K` で開くパネルでは、「プロセス終了済み」「入力待ちの候補」「ロック中（閉じられません）」に分類した一覧をチェックで選んで閉じます。**安全側に倒す設計**で、未送信の指示が残るペイン・5 分以内に出力があるペイン・表示中のペインは理由つきでロックし、AI 判定に関係なく閉じません。AI 呼び出しが失敗したときも「閉じない」側へ倒れます。閉じたペインは `Ctrl+Shift+T` で会話ごと復元できます。無名ペインには「ラベル案: <名前>」が出て「適用」で命名できます。

状態: 配信済み v0.21.25 / チェックボックス方式へ作り直し・AI 判定は提案のみ v0.21.29 / ダッシュボードへ移設 v0.40.0。

### 見守りと無応答検知 — 止まった委譲を 2 回連続で確かめてから知らせる

既定 10 分ごとに全ペインと台帳を巡回し、「判断待ち」「レート制限で待機中」「完了・未確認」「確認が必要な完了」「ログ未作成」「見守りの停滞」「タイムアウト」「出力停止」「未送信の入力」「無応答で停止」に分類します。右下に通知するのは、人の操作が要る 3 種類 (判断を待っている・作業が検収で不合格になった・送信されていない入力が残っている) だけです。通知は「開発 の「調査レーン」が判断を待っています」のようにワークスペース名とペイン名の文で出て、「開く」を押すとそのペインへ移動します。台帳で終わった便 (closed / done-verified-closed / abandoned / fallback-inline / lost) と、ペインが既に閉じている便は対象にしません。同じ件を 2 回連続で検知してから鳴らすので、一時的なもたつきでは通知しません。レート制限中のペインは休眠対象から外し、台帳でも「停止」ではなく「レート制限で待機中」として扱います (無駄な催促で枠を消費しないため)。質問ストリップの直下の行に「動作中 / 停止 / 計測なし」「最終チェック ○」「次回 ○」が出て、止まっているときは「ウィンドウ非表示のため停止」「子ウィンドウのため停止」「見守りがオフ」と理由まで出ます。間隔と停滞分数は設定「自動化」の「控えめ (30分 / 90分)」「標準 (10分 / 45分)」「敏感 (5分 / 20分)」「カスタム」から選びます。
...
状態: 見守り 配信済み v0.35.0 / ダッシュボード一覧への統合と抑止理由の可視化 v0.39.0 / 経過時間の丸め修正 v0.40.0 / 通知を人の操作が要る 3 種類と開いているペインに絞り、文面と「開く」ボタンを直した版 (次版)。無応答検知 配信済み v0.37.0 相当。
...
| 種別 | 拡張子 | 表示方法 |
| --- | --- | --- |
| HTML | `html` `htm` | 専用の子 WebView2 (別プロセス・JavaScript が動く・相対パスの画像も出る) |
...
HTML だけはペインの上に**専用の WebView2 を重ねて**描きます。別プロセスなので、大きいレポートを開いてもターミナルの描画が止まりません。文書の中の JavaScript が動き、相対パスで置いた画像も出ます。タブを切り替えても破棄されず、戻ると読んでいた位置のままです。開けるのは `asset.localhost` だけに絞ってあり、文書の中のリンクでペインが web サイトに乗っ取られることはありません (外部リンクは OS の既定ブラウザへ出ます)。ログイン画面も開けません。プロファイルは他と分けてあるので、成果物 HTML に他サービスのログイン情報は渡りません。**編集 (鉛筆ボタン) に入るときだけ従来の枠に戻ります** — 編集はペインから文書の中へ手を伸ばす作りで、別プロセスでは届かないためです。詳細は `docs/design/html-preview-webview.md`。
...
### 裏で進んだセッションに気づきたい → 通知バッジ

非アクティブなペインの出力変化を検知してペインのバッジを増やし、サイドバーのワークスペース行へ集約します。ペインにフォーカスすると消えます。v0.36.0 で対象を**「人の介在がないと進まない状態」だけ**に絞りました。`ls` が返っただけのコマンド完了や `> 1. ...` の番号付きリスト行では、もうバッジは増えません (完了はダッシュボード側の分類にだけ残ります)。全ペインの横断確認はダッシュボードで行います。

状態: 配信済み v0.1.0 / 対象の絞り込み v0.36.0 / 未読リングのサイドバー伝播 v0.21.31 / タイトルバーの通知ベルと通知パネルの撤去 (次版)。
...
| 見守りキューの通知 | 「いま気になっているペイン (N件)」 | 配信済み v0.35.0 |
...
| 接続 | スマホから使う | ポケットmycmux を開く QR と URL |
...
設定「AI」の「AI機能を有効にする」がマスタースイッチで、オフにすると下の機能がすべて止まります。「使用する AI」で Codex / Claude Code を選び、モデルはプロバイダ連動のプルダウン (カスタム入力も可) です。この設定で動くのは、①ペインの自動命名 ②返信案の準備 ③報告インボックスの要約 ④ペイン整理のAI判定 ⑤ailog セッション要約 ⑥ailog 一括要約 ⑦ペイン再配置（準備中） の 7 つで、①②はチェックで個別に切れます。⑦は「監査完了まで利用できません」の説明つきで**常時 disabled** です。状態: 配信済み v0.36.0 (AI のプロバイダ・モデル設定) / マスタースイッチと 7 機能一覧への再構成は 2026-08-25 (AISET 便) で master に入り、v0.57.0 には未配信 (次版)。
...
### スマホから見たい・承認だけ返したい → ポケットmycmux

設定「スマホから使う」に、ポケットmycmux を開く QR が出ます。読み取るとスマホのブラウザで開きます (ホーム画面に追加すればアプリのように使えます)。URL は「URLをコピー」でクリップボードへ入ります。iOS アプリ側にも同じ URL を受ける設定はありますが署名に載せていないため、いまは QR からアプリが直接開くことはありません。

QR が出るのは、この PC が `tailscale serve` で `/pocket` を配信しているときだけです。配信していなければ QR の代わりに案内が出ます。配信はされているのに応答が無いときは、URL の下に注意書きが出ます。

状態: mycmux 内蔵のモバイル UI (トークン付き URL と 7682 のサーバ) は 2026-09 に廃止し、スマホの入口をポケットmycmux へ一本化しました。mycmux 側が持つのは入口の QR だけで、アプリ本体・配信・認証はポケットmycmux 側にあります。
...
設定「キャラ」で、ワークスペースに 1 体出すか、まったく出さないかを選びます。候補カードのチェックを入れたものだけが新しいワークスペースのランダム抽選に出ます。新規ワークスペースのキャラは「候補からランダムで1体 (以後そのワークスペースに固定)」か「いつも同じ1体にする」から選べ、既存ワークスペースのキャラは「振り直す」「変更 ▸」で変えられます。`~/.codex/pets/<name>/{pet.json,spritesheet.webp}` に置いた Codex 規格の pet (8列×9行 / 8列×11行アトラス) をそのまま読み、「今すぐ再スキャン」で読み直します。「ギャラリーから探す (codex-pets.net)」で検索して 1 クリック取り込みもできます。規格外で読めない pet は「_disabled へ移す」で隔離でき (ファイルは削除しません)、隔離中の pet は「戻す」で候補へ返せます。スプライトは本家 Codex と同じ意味の 5 状態 (呼んでる=行 6 / 困ってる=行 5 / 走ってる=行 7 / 見て見て=行 8 / 寝てる=行 0) を再生します。
...
### ペイン再配置 (tab grouping) — 封印中

全ワークスペースのペインを分析して「案件」「役割」「移動最小」「複合」の再配置案を比較 → 内容を編集 → 適用前確認 → 適用まで行い、適用後は「元に戻す」で 1 回だけ巻き戻す機能です。ペインは閉じず PTY も殺しません (レイアウト操作だけです)。

状態: **開発中 (封印中)**。基盤コードは v0.57.0 に搭載済みですが、入口は `TAB_GROUPING_ENTRY_ENABLED = false` で未マウントです。独立監査が 2 ラウンド REJECT を出したため一度設計をやり直し、Gate 3 の UX 便 (legacy 削除 → 編集モデル → 編集可能な配置図 → Pointer DnD → 中断系 → 線の仕上げ) を順に消化しています (2026-08-27 時点で線の仕上げまで着地)。この後に実操作テスト・100 ペイン性能・実機試触・独立監査が続きます。**封印解除の条件**は Gate 6 の ACCEPT と、オーナーが実機で 4 シナリオを説明なしで完了することの両方です。決定済みの設計は、Pointer Events のみの DnD (外部ライブラリと HTML5 Drag API は使わない)、複数選択後に移動先タブをクリックでも移動可、グループのスイッチは「再配置する｜現状維持」の 2 択、編集内 1 手 undo と「AI 案に戻す」です。
...
| 5 | 上部右 5 アイコンが幅で消える | 完了 (ペインバーは幅で 7 段階に畳み、はみ出した操作は「⋮」へ移ります。畳む順は 新規ペイン・下に分割 → セーブポイント → ダッシュボード → ズーム → 右に分割 → 閉じる で、最後まで残るのは閉じるです) |
| 6 | 会話履歴が開けずダッシュボードへ飛ぶ | 完了 (`6ed846b`) |
| 7 | 再起動時のペイン復活が不安定 | ペイン再配置の後 (原因候補は特定済み) |
...
配置図に spawn 系譜 (母艦 → ↳作業者) の階層を重ね、working のペインだけ接続線とチップを微アニメーションさせ、完了・エラーは ✓ / ✗ で静める案です。**「動いてるものだけが動く」**が制約で、全体が常時ちらつく演出は作りません。色は意味のみ (working / idle / done / error)、テーマトークン準拠、`prefers-reduced-motion` で全アニメ停止、日本語 9px 床を守ります。状態: **開発中** (即時分はペイン再配置レーン Gate 3 のプレビュー描画に適用 / 配置図本体の生体化は独立レーン)。
...
| 機能 | 概要 | 状態 |
| --- | --- | --- |
| Web ペインの引き継ぎ書ワークフロー | brief.md と実ファイルをまとめて Web ペインへ渡す `gpt` ラッパーと、ファイル添付 (CDP `DOM.setFileInputFiles`) | 計画 (Web ペイン本体と `web.push` のテキスト投入は配信済み → [6 章](#6-エージェントを起動する)。ADR 0008) |
...
| フォーカスフォローマウス | マウスがタブに入ると自動でフォーカスする | 計画 (優先度 低) |
...
| 何を | いつ | 代わりに使うもの |
| --- | --- | --- |
| 内蔵のスマホ・リモート操作 (トークン付き URL・7682 のサーバ・専用モバイル UI) | 2026-09-19 | ポケットmycmux ([15 章](#15-スマホbuddyペット)) |
...
| タイトルバーのペイン整理・一括クローズボタン | v0.40.0 (バッジと常時スキャンは v0.21.29) | ダッシュボード配置図下の「ペイン掃除」 |
...
| ペイン掃除の常時スキャンとバッジ | v0.21.29 | 明示実行 + 提案のみの AI 判定 |
| コマンド完了による通知バッジ | v0.36.0 | ダッシュボード側の分類 |
| タイトルバーの通知ベルと通知パネル | 次版 | ペインのバッジとダッシュボードの横断一覧 |
...
| 裏ペインの画面を読むと内容が薄い | `pane.read` の背面ペイン読みは PTY スクロールバックのヘッドレス再生で、レンダラの表示そのものではありません。完了検知は画面ではなく DONE マーカー・rollout ログ・成果物の mtime で行ってください |
...
| 4 | Global | `tab.sweep` | `Ctrl+Shift+K` | ペイン掃除を開く |
...
| 24 | Pane | `pane.focus.left` | `Ctrl+Alt+ArrowLeft` | 左のタブへ |
| 25 | Pane | `pane.focus.right` | `Ctrl+Alt+ArrowRight` | 右のタブへ |
| 26 | Pane | `pane.focus.up` | `Ctrl+Alt+ArrowUp` | 上のタブへ |
| 27 | Pane | `pane.focus.down` | `Ctrl+Alt+ArrowDown` | 下のタブへ |
| 28 | Pane | `pane.split.right` | `Ctrl+Alt+D` | タブを右に分割 |
| 29 | Pane | `pane.split.down` | `Ctrl+Alt+Shift+D` | タブを下に分割 |
| 30 | Pane | `pane.close` | `Ctrl+Alt+W` | アクティブなタブを閉じる |
| 31 | Pane | `pane.reopen` | `Ctrl+Shift+T` | Reopen closed pane |
| 32 | Pane | `pane.zoom.toggle` | `Ctrl+Shift+Enter` | タブの最大化を切り替え |
| 33 | Pane | `pane.tab.next` | `Ctrl+Alt+PageDown` | タブ内の次のペイン |
| 34 | Pane | `pane.tab.prev` | `Ctrl+Alt+PageUp` | タブ内の前のペイン |
| 35 | Pane | `pane.attention.next` | `Ctrl+Alt+A` | Next attention |
| 36 | Pane | `pane.tab.pin.toggle` | `Ctrl+Alt+P` | アクティブペインをピン留め |
...
| 4 | `pane.list` | `list_panes` | フロント | 指定ワークスペースのタブ一覧 + レイアウト |
| 5 | `pane.list_all` | `list_all_panes` | フロント | 全ワークスペース横断のタブ・ペイン一覧 |
| 6 | `pane.spawn` | — | フロント | 新しいタブでエージェントを起動 (split) |
| 7 | `pane.spawn_tab` | — | フロント | 呼び出し元タブに裏ペインを開いて起動 (CLI の既定) |
| 8 | `pane.declare_tab` | — | フロント | PTY を起こさずペインだけ宣言する |
| 9 | `pane.launch_declared` | — | フロント | 宣言済みペインを実際に起動する (**現在フラグで無効**) |
| 10 | `pane.activate_tab` | — | フロント | ペインを内部的にアクティブ化 |
| 11 | `pane.restore_activation` | — | フロント | 以前の activation token を検証して復元 |
| 12 | `pane.close_tab` | — | フロント | ペインを 1 つ閉じる |
| 13 | `pane.close_tabs` | — | フロント | ペインを複数まとめて閉じる |
| 14 | `pane.rename_tab` | — | フロント | ペインのラベル変更 |
| 15 | `pane.send_text` | — | フロント (前段で Rust ガード) | 稼働中ターミナルへ文字列・キーを送る |
| 16 | `pane.read` | — | フロント | ターミナルバッファの読み取り (最大 400 行) |
| 17 | `pane.move` | — | フロント | タブをワークスペース内で移動 |
...
5 セクション / 12 タブ。ダイアログの aria-label は「設定」、閉じるボタンは「設定を閉じる」、ナビには「要再認証 {件数}」バッジが出ます。
...
| ランチャー | 新規に起動・Web の各行と「続きから」の表示チェック / フォルダ (開発・案件: 表示名の編集・表示・フォルダを選んで登録…・行の表示名編集・↑↓・固定・削除・無視) / 候補 (候補を更新・最終走査・開発に登録・案件に登録・無視・さらに N 件) / 自動登録 (ルールごとに 候補にする・自動で登録する・有効・編集・削除、ルールを追加 (4 種)、今すぐ走査) / 詳細 (正本 launch-dirs.json を開く・bash 用 launch-roots.txt と書き出し時刻・今すぐ書き出す・外部変更の取り込み結果・無視リストの解除) |
...
| スマホから使う | ポケットmycmux を開く QR と URL / URLをコピー / 更新 (配信していないときは案内) |
...
| 3 | `MYCMUX_TAB_ID` | ペイン寿命で安定な UUID。session id 採番と対応表の安定キー |
| 4 | `MYCMUX_RUNTIME_DIR` | ランタイム root (既定 `~/.mycmux`) |
| 5 | `MYCMUX_TEST_PROFILE` | テストプロファイル起動時に `"1"` |
| 6 | `MYCMUX_HTML_OUT` | HTML サイドペインの出力先 `<runtime>/sessions/<id>/out.html` |
...
| 20 | `MYCMUX_DISABLE_CLAUDE_AUTO_TRUST` | `"1"` で `~/.claude.json` への cwd 自動 trust を止める |
| 21 | `MYCMUX_SKIP_RAM_GATE` | `"1"` で `run_windows_tests.py` の RAM 待ちゲートを無効化 |
| 22 | `MYCMUX_RAM_GATE_MAX_WAIT_MIN` | 同ゲートの最大待ち時間 (分・既定 120) |
| 23 | `MYCMUX_GROUPING_LIVE_MOCK` | ペイングルーピング live mock のマーカー |
...
| `~/.mycmux/launch-dirs.json` (+ `.bak`) | ランチャーの「開発」「案件」の正本 (セクション・登録・無視リスト・自動登録ルール・最終走査)。設定 → ランチャー から編集 |
| `~/.mycmux/launch-roots.txt` (+ `.bak-YYYYMMDD`) | 上の台帳からアプリが書き出す派生ファイル (`表示名|パス`・bash 経路の `d` / `a` キー用)。旧版のファイルは初回に取り込まれ、外部で書き換えられた行も次の読み込みで取り込まれます。UTF-8 として読めないときは `.unreadable-<秒>` に退避してから書き直します (中身は消しません) |
...
| タブ | ワークスペースの中でグリッド状に並ぶ端末の区画 |
| ペイン | 1 つのタブの中に積まれる端末。非アクティブでも PTY は動き続ける |
...
| 可視ペイン | 委譲先のエージェントを、画面上のペインとして見える形で起動したもの |
| 裏ペイン | 呼び出し元タブに開くが、アクティブなペインは移動しない起動のしかた |
| 母艦 | 委譲元の親セッション。ここから可視ペインへ作業を降ろす |
...
| 5 | 07-10 → 08-03 | 40 | セーブポイントと `pane.spawn`。エージェントが可視ペインで別のエージェントを起動できるようになる |
| 6 | 08-04 → 08-11 | 44 | 8 日で 44 版。ソケットのトークン認証、CLI アカウント切替、右クリック凍結の根治 |
| 7 | 08-12 → 08-30 | 51 | ダッシュボードを 3 面の司令室へ。実行契約、質問カード、ペイン再配置、Web ペイン |
