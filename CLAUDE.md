...
- 日本語は **ワークスペース > タブ > ペイン**。**タブ = 画面を割った区画 1 つ** (コードの `Pane`・分割/最大化/移動/統合の対象)、**ペイン = セッション 1 つ** (コードの `PaneTab`・開閉/名前/固定/spawn/送信/読取の対象)
- **英語は日本語の鏡像**: 英語 `pane` = 日本語のタブ、英語 `tab` = 日本語のペイン。改名しない。**日本語を書くときは識別子から訳さない** (可視タブ・裏タブ・Web タブ・タブ掃除はすべてペインが正しい)
- 別義 (矛盾ではない): 設定画面のタブ / ブラウザのタブ / ダッシュボードの「2 ペイン」構成 / ポケット mycmux の「トーク」。2026-09-13 以前の記録には鏡像の語彙が混じる — 詳細は `docs/adr/0013-vocabulary-workspace-tab-pane.md`
...
- エージェント委譲規約: `docs/agent-integration.md` — mycmux 内の Codex/Claude/Grok 委譲は `scripts/mycmux_agent_cli.py spawn` で可視ペインを立てるのが正 (各エージェント側ルールの所在・新エージェント登録チェックリストもここに記載)
...
- **`cargo build` 単体は壊れた exe を作る** (frontend 未バンドル・19MB)。必ず `npm run tauri build`。**正常サイズは版とともに増える** — 2026-09-15 の v0.71.6 は稼働中の exe もローカルビルドも 45.6MB (旧記載の「32〜42MB」は 8/29 の 34.3MB 時代の値で、そのまま当てると正常なビルドを壊れていると誤判定する)。判定は「稼働中 exe (`%LOCALAPPDATA%\mycmux\mycmux.exe`) と同じ桁か」で行う
